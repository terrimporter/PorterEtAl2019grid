...
ggsave("FigS5_DNAextnRichness.pdf", p1, width=8, height=5, units="in")
...
kruskal.test( richness ~ site, data = summary_ext)
# Kruskal-Wallis chi-squared = 0.080435, df = 1, p-value = 0.7767 (n/s)

kruskal.test(richness ~ layer, data = summary_ext)
# Kruskal-Wallis chi-squared = 55.081, df = 2, p-value =1.095e-12 (sig)

kruskal.test(richness ~ as.factor(extractions), data = summary_ext)
# Kruskal-Wallis chi-squared = 0.40359, df = 1, p-value = 0.5252 (n/s)
