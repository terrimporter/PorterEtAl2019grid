...
# remove underscores between genus and species epithet
Zs$species <- gsub("_", " ", Zs$species)
...
ILC<-heat_tree(obj, 
          ...
          node_size_range = c(0.01,0.05), # create a larger range to emphasize differences in node sizes
          ...
          edge_size_range = c(0.005, 0.005), # make the edges thinner to emphasize differences in node sizes
...
ggsave("Fig4_two_heattrees.pdf", g)
...
ggsave("FigS9_layers_heattrees.pdf", h, height=10, width=7.5, units = "in")
...
ggsave("FigS8_two_extraction_heattrees.pdf", i)
...
ggsave("FigS7_pooled_heattrees.pdf", j, height=10, width=7.5, units = "in")
